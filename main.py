...
def get_colors(img, num_of_colors):
    ...
    manipulated_img = manipulated_img.reshape(
        manipulated_img.shape[0] * manipulated_img.shape[1], 3)
...
def hex_to_color_name(most_frequent_color):
    ...
    most_frequent_rgb = np.asarray([int(most_frequent_color[1:3], 16), int(
        most_frequent_color[3:5], 16), int(most_frequent_color[5:7], 16)], np.uint8)
    most_frequent_rgb = np.dstack(
        (most_frequent_rgb[0], most_frequent_rgb[1], most_frequent_rgb[2]))
    ...
    lab_dist = ((lab[:, :, 0] - most_frequent_lab[:, :, 0])**2 + (lab[:, :, 1] -
                most_frequent_lab[:, :, 1])**2 + (lab[:, :, 2] - most_frequent_lab[:, :, 2])**2)**0.5
...
def colorDetectView():
    try:
        ...
        for i in range(len(files)):
            ...
            file_names_paths.append({"filepath": os.path.join(
                path, files[i].filename), "filename": files[i].filename})
        responseArray = []
