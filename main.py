...
# function to get the hex values of the colors
def RGB2HEX(color):
    return "#{:02x}{:02x}{:02x}".format(int(color[0]), int(color[1]), int(color[2]))


# function to fetch the top colors (hex codes) from the img
def get_colors(img, num_of_colors):

    manipulated_img = cv2.resize(img, (600, 400), interpolation=cv2.INTER_AREA)
    manipulated_img = manipulated_img.reshape(manipulated_img.shape[0] * manipulated_img.shape[1], 3)

    clf = KMeans(n_clusters = num_of_colors)
    labels = clf.fit_predict(manipulated_img)
...
# function to convert the hex codes to color names
def hex_to_color_name(most_frequent_color):
    # define a dictionary of colors hex codes mapping to colors names
    colors_dict = {
    ...
    # Get a list of colors hex code in string format
    ...
    r = np.asarray(r, np.uint8)  # Convert r from list to array
    g = np.asarray(g, np.uint8)  # Convert g from list to array
    b = np.asarray(b, np.uint8)  # Convert b from list to array

    rgb = np.dstack((r, g, b)) #Stack r,g,b across third dimention

    lab = color.rgb2lab(rgb)

    most_frequent_rgb = np.asarray([int(most_frequent_color[1:3], 16), int(most_frequent_color[3:5], 16), int(most_frequent_color[5:7], 16)], np.uint8)
    most_frequent_rgb = np.dstack((most_frequent_rgb[0], most_frequent_rgb[1], most_frequent_rgb[2]))
    most_frequent_lab = color.rgb2lab(most_frequent_rgb)

    # Compute Euclidean distance from most_frequent_lab to each element of lab
    lab_dist = ((lab[:,:,0] - most_frequent_lab[:,:,0])**2 + (lab[:,:,1] - most_frequent_lab[:,:,1])**2 + (lab[:,:,2] - most_frequent_lab[:,:,2])**2)**0.5

    # Get the index of the minimum distance
    min_index = lab_dist.argmin()

    # Get hex string of the color with the minimum Euclidean distance
    most_frequent_closest_hex = hex_rgb_colors[min_index]

    most_frequent_color_name = colors_dict[most_frequent_closest_hex]
    return most_frequent_color_name


def main(path, fileName):
    try:
        img = get_img(path)
        hex_code = get_colors(img, 2)
        most_frequent_color_code = hex_code[0]
        print("Hex Code: " + most_frequent_color_code)
        most_frequent_color = hex_to_color_name(most_frequent_color_code)
        print("Color Name: " + most_frequent_color)
        resData = {
            'hexCode' : most_frequent_color_code,
            'colorName' : most_frequent_color,
            'Name' : fileName
        }
        return resData
    except:
...
@app.route('/colorDetect',methods = ['POST', "OPTIONS"])
def colorDetectView():
    try:
        if 'file' not in request.files:
            return Response('Please Upload an Img', status = 400)    
        docx_array = []
        
        files = request.files.getlist("file")
        file_names_paths = []
        for file in files:
            docx_array.append(file.filename.replace('pdf','docx'))
        for i in range(len(files)):
            files[i].save(os.path.join(files[i].filename))
            file_names_paths.append({"filepath" : os.path.join(path,files[i].filename) , "filename" : files[i].filename})
        responseArray = []
        for fileNameTemp in file_names_paths:
            result = main(fileNameTemp['filepath'] , fileNameTemp['filename'])
            responseArray.append(result)
        try:
            ext = ['.png','.jpeg',',jpg','.jfif','.svg']
            for file in os.listdir(path):
                if file.endswith(tuple(ext)):
                    os.remove(file) 
        except:
            print("unable to Remove images")
        return jsonify(results=responseArray)    
